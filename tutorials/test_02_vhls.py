...
def gemmT(A: float32[M, N], B: float32[M, N]) -> float32[M, M]:
    C: float32[M, M] = 0.0
    for i, j in allo.grid(M, N):
        for k in allo.reduction(N):
            C[i, j] += A[i, k] * B[j, k]
    return C

def assign(A: float32[M]) -> float32[M]:
    B: float32[M] = 0.0
    for i in allo.grid(M):
        B[i] = A[i]
    return B

def sftm(A: float32[M, M]) -> float32[M, M]:
    B = allo.library.nn.softmax[float32, M](A)
    return B

FUNC = sftm

def dotm(A: float32[M, N], B: float32[M, N]) -> float32[M, N]:
    C: float32[M, N] = 0.0
    for i, j in allo.grid(M, N):
        C[i, j] = A[i, j] * B[i, j]
    return C

def attn(Q: float32[M, N], K: float32[M, N], V: float32[M, N]) -> float32[M, N]:
    # R: float3i2[M, N] = 0.0
    # QKT: float32[M, M] = 0.0
    rowsum: float32 = 0.0
    f_M: float32 = M
    ...
    for i in allo.grid(M):
        rowsum = 0.0
        for j in allo.reduction(M):
            QKT[i, j] = QKT[i, j] / allo.sqrt(f_M)
            rowsum += allo.exp(QKT[i, j])
        for j in allo.grid(M):
            QKT[i, j] = allo.exp(QKT[i, j]) / rowsum
...
# mod = s.build(target="vivado_hls", mode="hw", project="attn.prj")
