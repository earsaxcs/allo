...
def qlinear(X, W, fscl, iscl, oscl, wscl, bscl, izr=None, ozr=None, bias=None, name=None):
    if bias is None:
        return matmul(X, W.T)
    return matmul(X, W.T) + bias

def qconv2d(inp, filter, _stride, fscl, iscl, oscl, wscl, bscl, izr=None, ozr=None, bias=None, name=None):
    view_shape = (
        tuple(inp.shape[:2]) # B, IC
        + tuple(np.subtract(inp.shape[2:], filter.shape[2:]) // _stride + 1) # (H - KH) / S + 1, (W - KW) / S + 1
        + filter.shape[2:] # KH, KW
...
def igelu(x, fscl, iscl, oscl, izr=None, ozr=None, name=None):
    return 0.5 * x * (1 + np.tanh(np.sqrt(2 / np.pi) * (x + 0.044715 * x**3)))

def isoftmax(x, fscl, iscl, oscl, izr=None, ozr=None, name=None):
    exp_x = np.exp(x - np.max(x, axis=-1, keepdims=True))
    return exp_x / np.sum(exp_x, axis=-1, keepdims=True)

def ilayernorm(x, gamma, beta, fscl, iscl, oscl, izr=None, ozr=None, eps: float = 1e-5):
    mean = np.mean(x, axis=-1, keepdims=True)
    variance = np.var(x, axis=-1, keepdims=True)
    x = gamma * (x - mean) / np.sqrt(variance + eps) + beta
    return x

def qmatmul(lhs, rhs, fscl, iscl, oscl, izr=None, ozr=None, name=None):
    # coe can either carry or not carry isqrtd, just depends on whether it comes from (x * w / o) / sqrtd? 
    # so you can just change coe to carry isqrtd or not
    return np.matmul(lhs, rhs)

def qadd(lhs, rhs, lfscl, rfscl, iscl, oscl, izr=None, ozr=None, name=None):
    return lhs + rhs